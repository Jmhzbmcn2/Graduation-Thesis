...
class LightRAG:
    ...
    async def aquery_data(
        ...
        if data_param.mode in ["local", "global", "hybrid", "mix", "beam", "focused", "ivf_local", "ivf_focused"]:
    ...
    async def aquery_llm(
        ...
        try:
            ...
            if param.mode in ["local", "global", "hybrid", "mix", "beam", "focused", "ivf_local", "ivf_focused"]:
                query_result = await kg_query(
                    query.strip(),
                    self.chunk_entity_relation_graph,
                    self.entities_vdb,
